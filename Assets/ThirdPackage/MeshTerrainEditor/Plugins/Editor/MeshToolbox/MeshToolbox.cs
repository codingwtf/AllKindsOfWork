    ...
    internal class MeshToolbox : IEditor
    {
        ...
        public float BrushSize
        {
            ...
            set
            {
                ...
                if (!MathEx.AlmostEqual(value, brushSize))
                {
                    brushSize = value;
        ...
        public void DoArgsGUI()
        {
            ...
            if (mode == SelectMode.Vertex)
            {
                ...
                {
                    GUI.enabled = CanSnap();
                    if (GUILayout.Button(StringTable.Get(C.Snap), GUILayout.Width(100), GUILayout.Height(40)))
                    {
                        Snap();
                    }
                    GUI.enabled = true;
                    GUILayout.Space(20);
                    EditorGUILayout.LabelField(StringTable.Get(C.Info_MeshToolBox_Snap), MTEStyles.labelFieldWordwrap);
                }
                EditorGUILayout.EndHorizontal();
            }
        ...
        private void OnSceneGUIForQuad()
        {
                ...
                ))
            {
                ...
                if (pressed)
                {
                    ...
                    MeshSaving.UpdateMeshColliderImmediately(meshCollider);
        ...
        private void TranslateVertices(Vector3 offset)
        {
            ...
            for (var i = 0; i < vertexModifyGroups.Count; i++)
            {
                ...
                MeshSaving.SetMeshDirty(gameObject);
        ...
        bool CanDelete() => SelectedAnyVertex();
        ...
        private bool CanSnap() => SelectedAnyVertex();

        void Snap()
        {
            //convert to world space
            var worldVertices = new List<WorldVertex>();
            foreach (var modifyGroup in vertexModifyGroups)
            {
                var gameObject = modifyGroup.Target;
                var mesh = gameObject.GetComponent<MeshFilter>().sharedMesh;
                var vertices = mesh.vertices;
                foreach (var vertexIndex in modifyGroup.VertexIndexList)
                {
                    var vertexPosition = vertices[vertexIndex];
                    var worldVertex = new WorldVertex(modifyGroup.Target, mesh, vertexIndex, vertexPosition);
            ...
            //compare and calculate average position
            //collection all modifications to the mesh(es)
            HashSet<GameObject> involvedGameObjects = new HashSet<GameObject>();
            Dictionary<Mesh, PositionModificationData> modifications = new Dictionary<Mesh, PositionModificationData>();
            for (var i = 0; i < worldVertices.Count; i++)
            {
                var a = worldVertices[i];
                for (var j = i + 1; j < worldVertices.Count; j++)
                {
                    var b = worldVertices[j];
                    if (!MathEx.AlmostEqual(a.WorldPosition.x, b.WorldPosition.x)
                        || !MathEx.AlmostEqual(a.WorldPosition.z, b.WorldPosition.z))
                    {
                        continue;
                    }

                    var averageY = (float)(0.5 * (a.WorldPosition.y + b.WorldPosition.y));
                    var aWorldPosition = a.WorldPosition;
                    aWorldPosition.y = averageY;
                    var bWorldPosition = b.WorldPosition;
                    bWorldPosition.y = averageY;

                    var aLocalPosition = a.GameObject.transform.InverseTransformPoint(aWorldPosition);
                    var bLocalPosition = b.GameObject.transform.InverseTransformPoint(bWorldPosition);

                    if (!modifications.ContainsKey(a.Mesh))
                    {
                        modifications.Add(a.Mesh, new PositionModificationData());
                    }
                    if (!modifications.ContainsKey(b.Mesh))
                    {
                        modifications.Add(b.Mesh, new PositionModificationData());
                    }

                    var modA = modifications[a.Mesh];
                    modA.indices.Add(a.Index);
                    modA.positions.Add(aLocalPosition);
                    var modB = modifications[b.Mesh];
                    modB.indices.Add(b.Index);
                    modB.positions.Add(bLocalPosition);

                    involvedGameObjects.Add(a.GameObject);
                    involvedGameObjects.Add(b.GameObject);
                }
            }

            using (new Undo.UndoTransaction("MeshToolBox Snap Vertices"))
            {
                // modify mesh in batch
                foreach (var pair in modifications)
                {
                    var mesh = pair.Key;
                    var indices = pair.Value.indices;
                    var positions = pair.Value.positions;
                    var vertices = mesh.vertices;
                    ...
                    mesh.ModifyPositions("Snap Vertices", vertices);
                }
            }

            foreach (var involvedGameObject in involvedGameObjects)
            {
                VertexMap.Rebuild(involvedGameObject);
                MeshSaving.SetMeshDirty(involvedGameObject);
            }
        }
    }
}