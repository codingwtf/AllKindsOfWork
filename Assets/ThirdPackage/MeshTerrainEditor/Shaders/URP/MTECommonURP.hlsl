...
float MipMapLevel(float2 uvInTexel)
{
	float2 dx_vtc = ddx(uvInTexel);
	float2 dy_vtc = ddy(uvInTexel);
...
float4 MTE_SampleTextureArray(Texture2DArray array, SamplerState SS, float texelSize, int layerIndex, float2 uv, float LayerUVScales[12], float UVScale)
{
#ifdef ENABLE_LAYER_UV_SCALE
    float layerUVScale = LayerUVScales[layerIndex];
    float2 scaledUV = uv * layerUVScale;
    float mipLevel = MipMapLevel(scaledUV * texelSize);
    return SAMPLE_TEXTURE2D_ARRAY_LOD(array, SS, scaledUV, layerIndex, mipLevel);
#else
