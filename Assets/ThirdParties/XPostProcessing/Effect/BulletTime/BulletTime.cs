...
namespace IGG.Rendering.XPostProcessing
{
    ...
    public class BulletTimeRenderer : VolumeRenderer<BulletTime>
    {
        ...
        {
            if (volumeSettings.m_TimeReset)
            {
                timeCount = 0;
                volumeSettings.m_TimeReset = false;
            }

            if (timeCount > volumeSettings.bulletTimesTime.value)
            {
                return false;
            }
            else
            ...
            //if(timeCount >= volumeSettings.bulletTimesTime.value)return false;
            return base.IsActive();
        }

        public override void Init()
        {
            shader = FindShader.Find("Hidden/PostProcessing/BulletTime");
        ...
        public override void Render(CommandBuffer cmd, RenderTargetIdentifier source, RenderTargetIdentifier target)
        {
            ...
            vec.w = timeCount * volumeSettings.bulletTimesBloomSpeed.value;
            m_BliteMaterial.SetVector(ShaderContants.bulletTimeOriginPositionID, vec);
            m_BliteMaterial.SetFloat(ShaderContants.bulletTimeBlurPowerID, volumeSettings.bulletTimeRadialBlurPower.value);
            m_BliteMaterial.SetInt(ShaderContants.bulletTimeBlurQualityID, volumeSettings.bulletTimeRadialBlurQuality.value);
            m_BliteMaterial.SetVector(ShaderContants.bulletTimeBlurCurveID, volumeSettings.bulletTimeColor.value);
