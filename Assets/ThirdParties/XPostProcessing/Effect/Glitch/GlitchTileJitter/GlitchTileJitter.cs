...
namespace IGG.Rendering.XPostProcessing
{
    ...
    public class GlitchTileJitterRenderer : VolumeRenderer<GlitchTileJitter>
    {
        ...
        {
            shader = FindShader.Find("Hidden/PostProcessing/Glitch/TileJitter");
        ...
        public override void Render(CommandBuffer cmd, RenderTargetIdentifier source, RenderTargetIdentifier target)
        {
            ...
            if (volumeSettings.jitterDirection.value == Direction.Horizontal)
            {
            ...
            m_BlitMaterial.SetVector(ShaderIDs.Params, new Vector4(volumeSettings.splittingNumber.value, volumeSettings.amount.value, volumeSettings.speed.value * 100f,
                 volumeSettings.intervalType.value == IntervalType.Random ? randomFrequency : volumeSettings.frequency.value));

            cmd.Blit(source, target, m_BlitMaterial, volumeSettings.splittingDirection.value == Direction.Horizontal ? 0 : 1);

            cmd.EndSample(PROFILER_TAG);
        }

        void UpdateFrequency(GlitchTileJitter volumeSettings)
        {
