...
namespace IGG.Rendering.XPostProcessing
{
    ...
    public class RaderWaveRenderer : VolumeRenderer<RaderWave>
    {
        ...
        {
            if (volumeSettings.TimeReset)
            {
                timeCount = 0;
                volumeSettings.TimeReset = false;
            }

            if (timeCount > volumeSettings.raderWaveTime.value)
            {
                return false;
            }
            else
        ...
        {
            shader = FindShader.Find("Hidden/PostProcessing/RaderWave");
        ...
        public override void Render(CommandBuffer cmd, RenderTargetIdentifier source, RenderTargetIdentifier target)
        {
            ...
            // 			// vec.w = timeCount * volumeSettings.raderWaveSpeed.value;
            // 			// m_BlitMaterial.SetVector(ShaderContants.raderOriginPosID,vec);
            // #endif
            Vector4 vec = volumeSettings.OriginPos;
            vec.w = timeCount * volumeSettings.raderWaveSpeed.value;
            m_BlitMaterial.SetVector(ShaderContants.raderOriginPosID, vec);
            m_BlitMaterial.SetVector(ShaderContants.raderWaveColorID, volumeSettings.raderWaveColor.value);
            //m_BlitMaterial.SetFloat(ShaderContants.raderMaxDistanceID,volumeSettings.raderWaveMaxDistance.value);
            m_BlitMaterial.SetFloat(ShaderContants.raderWaveWidthID, volumeSettings.raderWaveWidth.value);
            m_BlitMaterial.SetVector(ShaderContants.raderWaveAreaColorID, volumeSettings.raderAreaColor.value);
